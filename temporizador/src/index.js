...
function executeChronometer() {
  ...
  function startTimer() {
    ...
    minutesSpan.textContent = minutes; //SE ESCRIBA EN EL DOCUMENTO 
    secondsSpan.textContent = seconds;
    secondsValue = seconds;
    minutesValue = minutes;
  
    currentInterval = setInterval(() => {
      secondsValue -= 1;//SE LE VA IR RESTANDO 1 AL TEMPORIZADOR 
      if (secondsValue === -1) { //SE VA A RESETER DE 1 MINUTO A 59 SECONDS
        secondsValue = 59;
        minutesValue -= 1;
      }
      if (minutesValue === 0 && secondsValue === 0) {
